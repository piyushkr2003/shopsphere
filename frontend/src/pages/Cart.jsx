    ...
    return (
      <div className="flex-grow flex items-center justify-center bg-slate-950 py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-md w-full text-center space-y-8 bg-slate-900 p-8 sm:p-10 rounded-3xl border border-slate-800 shadow-2xl">
          ...
          <div>
            <h2 className="text-3xl font-extrabold text-white tracking-tight">Your Cart is Empty</h2>
  ...
  return (
    <div className="flex-grow bg-slate-950 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-extrabold text-white mb-10 tracking-tight">Shopping Cart</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            {cart.map((item) => {
              ...
              return (
                <div 
                  key={itemId}
                  className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-6 bg-slate-900 border border-slate-800 rounded-3xl gap-6 hover:border-slate-700 transition-colors duration-200"
                >
                  {/* Product Info */}
                  <div className="flex items-center gap-6 w-full sm:w-auto">
                    <img 
                      ...
                      alt={item.name} 
                      className="w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-2xl border border-slate-850 flex-shrink-0 bg-slate-950"
                    />
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-indigo-400 uppercase tracking-wider mb-1">
                        {item.category}
                      </p>
                      <h3 className="text-lg sm:text-xl font-bold text-white truncate pr-4">
                        {item.name}
                      </h3>
                      <p className="text-slate-400 text-sm mt-1 sm:mt-1.5 font-medium">
                  ...
                  {/* Quantity & Actions */}
                  <div className="flex items-center justify-between sm:justify-end gap-8 w-full sm:w-auto border-t sm:border-t-0 border-slate-800/80 pt-4 sm:pt-0">
                    {/* Quantity Selector */}
                    <div className="flex items-center bg-slate-950 border border-slate-800 rounded-xl px-2 py-1">
                      <button 
                        onClick={() => handleDecrement(item)}
                        className="text-slate-400 hover:text-white p-1.5 transition-colors focus:outline-none"
                      ...
                      <span className="text-white font-bold px-4 text-base w-8 text-center select-none">
                        {item.quantity}
                      </span>
                      <button 
                    ...
                    <button 
                      ...
                      className="text-slate-500 hover:text-red-450 p-2 rounded-xl hover:bg-red-500/10 transition-all focus:outline-none"
          ...
          <div className="lg:col-span-1">
            <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl sticky top-28 space-y-6">
              <h2 className="text-2xl font-bold text-white pb-4 border-b border-slate-800">Order Summary</h2>
              ...
              <div className="space-y-4 text-slate-300 font-medium">
                ...
                <div className="flex justify-between text-lg font-bold pt-4 border-t border-slate-800/80">
