  ...
  const renderStars = (rating = 4.5) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= fullStars) {
        ...
        stars.push(
          <svg key={i} className="h-5 w-5 text-slate-600" viewBox="0 0 20 20" fill="currentColor">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        );
      }
    ...
    return (
      <div className="flex-grow flex items-center justify-center bg-slate-950 py-24">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-slate-400 text-sm font-semibold">Loading product details...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-grow bg-slate-950 py-16 px-4 sm:px-6 lg:px-8 flex items-center justify-center">
        <div className="max-w-md w-full text-center space-y-6 bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-2">{error || "We couldn't retrieve the specified item details."}</p>
          </div>
          <button 
            onClick={() => navigate('/')}
  ...
  return (
    <div className="flex-grow bg-slate-950 py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <Link 
          to="/"
          className="inline-flex items-center text-slate-400 hover:text-white font-bold mb-10 transition-colors gap-2"
        >
        ...
        {/* Product Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl">
          
          {/* Left Column: Image wrapper */}
          <div className="flex items-center justify-center bg-slate-950/60 border border-slate-850 rounded-2xl p-4 sm:p-8 shadow-inner relative overflow-hidden group">
            <img 
              src={product.imageUrl || product.image} 
              alt={product.name} 
              className="w-full h-auto max-h-[480px] object-contain rounded-xl transform group-hover:scale-102 transition-transform duration-500 ease-out"
            />
            <div className="absolute top-6 right-6 bg-slate-950/80 backdrop-blur-md px-3.5 py-2 rounded-full text-sm font-bold text-white flex items-center shadow border border-slate-800">
          ...
          <div className="flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              <span className="inline-block text-sm font-bold text-indigo-400 uppercase tracking-widest bg-indigo-500/10 px-3.5 py-1.5 rounded-xl border border-indigo-500/20">
                {product.category}
              </span>
              ...
              <div className="border-t border-slate-800 pt-6">
                ...
                <p className="text-slate-350 font-medium text-base sm:text-lg leading-relaxed">
                  {product.description}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6 pt-6 border-t border-slate-800">
              ...
              {!isOutOfStock && (
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                  <div className="flex items-center justify-between sm:justify-start bg-slate-950 border border-slate-800 rounded-xl px-2 py-1 flex-shrink-0">
                    <button 
                      onClick={handleDecrement}
                      className="text-slate-400 hover:text-white p-2.5 transition-colors focus:outline-none"
                    ...
                    <span className="text-white font-bold px-6 text-lg w-12 text-center select-none">
                      {qty}
                    </span>
                    <button 
        ...
        {relatedProducts.length > 0 && (
          <div className="mt-20">
            <h2 className="text-3xl font-extrabold text-white mb-10 tracking-tight">Related Products</h2>
